...
#all outliers are points not included in the whiskers of the boxplot
```
...
#use sum to add together all the probability values (dbinom) for 7, 8, 9, 10. if you dont use sum it will produce a list that you will end up having to sum later
...
#there is a higher probability that you will fin
```
...
```{r}
### Write your code here
#poission for 5 occurrences
dpois(5, 3)

#poission for less than 3 occurrences
sum(dpois(0:3, 3))
#or ppois(3, 3)
...
#header of column 1 is "Value" with the numbers from list 1; Group is the second column that makes all values group = List 1
List1 <- data.frame(
  Value = list1, 
  Group = "List 1"
)
...
I think these two lists come from different distributions because the mean is slightly different with List 2 being slightly lower, but the value range is much greater in List 2, meaning the standard deviation would be higher.
